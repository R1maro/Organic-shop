...
import {getSettings , getSettingGroups} from '@/utils/dashboard/setting';
import SearchForm from "@/components/Settings/SearchForm";
import GroupSelector from "@/components/Settings/GroupSelector";
import config from "@/config/config";

async function SettingsList({page = 1, group, search,}: {
    page?: number;
    group?: string;
    search?: string;
}) {
    const [settings, groups] = await Promise.all([
        getSettings(page, group, search),
        getSettingGroups()
    ]);
            ...
            <div className="mb-6 flex flex-col gap-3">
                <div className="mb-6 flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        <SearchForm initialSearch={search} group={group}/>
                        <GroupSelector groups={groups} defaultValue={group} search={search}/>
                    </div>
                </div>
            </div>
