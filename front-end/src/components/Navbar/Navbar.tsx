...
const Navbar = () => {
    ...
    useEffect(() => {
        const fetchLogo = async () => {
            try {
                const response = await fetch(`${config.API_URL}/settings/logo/`);
    ...
    return (
        <>
            ...
            <div className="header px-10">
                ...
                <img className="logo" src={`${config.PUBLIC_URL}${logoUrl}`}/>
                {!searchActive && (
                    <>
                ...
                <div className="header-icons">
                    <div className={`search-container ${searchActive ? "search-active" : ""}`}>
                        <span className="search" onClick={() => setSearchActive(!searchActive)}>
                             <svg fill="none" stroke="currentColor" viewBox="0 0 22 22"
